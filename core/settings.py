...
class APISettings(BaseSettings):
    ...
    NO_VERIFY_URL: Dict = {
        ...
        "/user/not": "in",  # 注册
        "/ws/": "in" , # ws 服务不需要登录
        "/verfic/": "in" , # ws 服务不需要登录
        "/user/download/point": "eq",  # ws 服务不需要登录
        "/user/userimage_url": "eq",  # ws 服务不需要登录
        "/user/showimage": "eq",  # ws 服务不需要登录
        "/user/avatar": "eq",  # ws 服务不需要登录
        "/user/browse_code": "eq",  # ws 服务不需要登录
        "/user/browse/point": "eq",  # ws 服务不需要登录
        "/user/subscribe": "eq",  # ws 服务不需要登录
        "/ms/message/receive": "eq",  # ws 服务不需要登录
    }
    BASE_DIR: Path = BASE_PATH.joinpath('templates/assets')
...
DATABASE_CONFIG: Dict = {
    ...
    'apps': {
        'models': {
            # 设置key值“default”的数据库连接
            'default_connection': 'default',
            'models': ['aerich.models','apps.user.model', 'casbin_tortoise_adapter']
        }
    },
    "routers": ["db_router.Router"],
    'use_tz': False,
